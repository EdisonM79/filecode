...
<div class="pd-20">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a class="btn btn-primary radius" onclick="employee_add('项目录入','${pageContext.request.contextPath}/admin/three/add')" href="javascript:;">
	...
	<div class="mt-20 input-father">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			<c:forEach items="${threeValues}" var="three">  
				<tr class="text-c">
					<td>${three.threeId}</td>
					<td>
						<c:if test="${three.department=='YF'}">研发部</c:if>
						<c:if test="${three.department=='SC'}">生产部</c:if>
						<c:if test="${three.department=='ZL'}">质量部</c:if>
						<c:if test="${three.department=='CW'}">财务部</c:if>
					...
					<td>${three.threeNum}</td>
					<td>${three.userName}</td>
					<td>${three.createTime}</td>
