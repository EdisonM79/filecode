...
<title>二级文件表单管理</title>
</head>
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 文件管理 <span class="c-gray en">&gt;</span> 二级文件表单管理
...
<div class="pd-20">
	...
	<div class="mt-20 input-father">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			<c:forEach items="${twotableValues}" var="twotable">  
				<tr class="text-c">
					<td>${twotable.twotableId}</td>
					<td>
						<c:if test="${twotable.department=='YF'}">研发部</c:if>
						<c:if test="${twotable.department=='SC'}">生产部</c:if>
						<c:if test="${twotable.department=='ZL'}">质量部</c:if>
						<c:if test="${twotable.department=='CW'}">财务部</c:if>
					...
					<td>${twotable.tableName}</td>
					<td>${twotable.username}</td>
					<td>${twotable.createTime}</td>
