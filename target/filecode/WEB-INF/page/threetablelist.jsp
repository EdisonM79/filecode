...
<title>三级文件表单管理</title>
</head>
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 文件管理 <span class="c-gray en">&gt;</span> 三级文件表单管理
...
<div class="pd-20">
	...
	<div class="mt-20 input-father">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			<c:forEach items="${threetableValues}" var="threetable">  
				<tr class="text-c">
					<td>${threetable.threetableId}</td>
					<td>
						<c:if test="${threetable.department=='YF'}">研发部</c:if>
						<c:if test="${threetable.department=='SC'}">生产部</c:if>
						<c:if test="${threetable.department=='ZL'}">质量部</c:if>
						<c:if test="${threetable.department=='CW'}">财务部</c:if>
						<c:if test="${threetable.department=='ZH'}">综合部</c:if>
						<c:if test="${threetable.department=='YX'}">营销部</c:if>
					</td>
					<td>${threetable.threeName}</td>
					<td>${threetable.threetableNum}</td>
					<td>${threetable.threetableVersion}</td>
					<td>${threetable.threetableCode}</td>
					<td>${threetable.threetableName}</td>
					<td>${threetable.userName}</td>
					<td>${threetable.createTime}</td>
